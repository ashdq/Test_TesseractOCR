...
# Konfigurasi
IMAGE_FOLDER = 'uploads/sample kk' # Folder berisi gambar Kartu Keluarga
OUTPUT_FILE = 'dataset_kk.json'    # Nama file output dibedakan agar tidak menimpa data KTP

# Daftar field yang ingin dijadikan kunci jawaban (hanya Nomor KK)
FIELDS_TO_EXTRACT = [
    "nomor_kk"
]

def create_dataset_kk():
    print("=== Sistem Pembuat Ground Truth KK (dataset_kk.json) ===\n")
    ...
    if not os.path.exists(IMAGE_FOLDER):
        print(f"Folder '{IMAGE_FOLDER}' tidak ditemukan. Buat foldernya dan masukkan gambar KK.")
        return
    ...
    if not images:
        print(f"Tidak ada gambar KK (jpg/jpeg/png) di dalam folder '{IMAGE_FOLDER}'.")
        return

    print(f"Ditemukan {len(images)} gambar KK. Memulai proses...\n")

    for img_name in images:
        # Format path: misal "uploads/sample kk/kk1.jpg"
        img_path = f"{IMAGE_FOLDER}/{img_name}" 
        
        # Skip jika gambar ini sudah ada di dataset_kk.json
        ...
        for field in FIELDS_TO_EXTRACT:
            ...
            if user_input:  # Hanya masukkan ke JSON jika tidak dikosongkan
                # Menghapus spasi atau tanda baca jika pengguna tidak sengaja mengetiknya
                clean_input = ''.join(filter(str.isdigit, user_input)) 
                ground_truth[field] = clean_input 
    ...
    print(f"\nSelesai! File kunci jawaban KK telah diperbarui di: {OUTPUT_FILE}")
